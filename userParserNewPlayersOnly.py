...
print(len(usersFileLines))
for line in usersFileLines:    

    if line == "uplay,Lucky_Arian\n":
        print("me")
...
for user in users:
    ...
    try:
        ...
        if data[f"for-honor|{platform}|{lowerCaseUN}"]["status"] != 0 and not skipUser: 
            skipUser = True
            failedUsersFile = open("failedUsers.csv","a")
            failedUsersFile.write(platform + "," + username + "\n")
            failedUsersFile.close()
    ...
    if not skipUser:
        ...
        if not skipUser:
            ...
            if(num % 100 == 0):
                dataFile = open(f"C:\\Users\\Jack Bowman\\Documents\\Programs\\PytScripts\\UserScraper\\datafiles\\data{str(num)}NEW.json","a")
...
dataFile = open(f"C:\\Users\\Jack Bowman\\Documents\\Programs\\PytScripts\\UserScraper\\datafiles\\dataFinal.json","a")
dataFile.write(json.dumps(players, indent=4))
dataFile.close()
players = {}
