...
def getUsersByModeProp(DuelFlag=False,proportion=1.5):
    sqlStr = """
    SELECT name,username, (wins + losses) from 
    (
    SELECT max_date.name,max_date.username,(max_date.wins - min_date.wins) as wins,(max_date.losses - min_date.losses) as losses from

    (SELECT * FROM
    (SELECT DomNDuel.playerID, DomNDuel.name,DomNDuel.wins,DomNDuel.losses,stat.username,stat.UTCSeconds,max(UTCSeconds) over (partition by username) as max_date
    from (SELECT * from mode where name='Duel' or name='Dominion') DomNDuel 
    INNER JOIN stat on stat.playerID=DomNDuel.playerID) DomNDuel
    WHERE UTCSeconds=max_date) max_date

    INNER JOIN

    (SELECT * FROM
    (SELECT DomNDuel.playerID, DomNDuel.name,DomNDuel.wins,DomNDuel.losses,stat.username,stat.UTCSeconds,min(UTCSeconds) over (partition by username) as min_date
    from (SELECT * from mode where name='Duel' or name='Dominion') DomNDuel 
    INNER JOIN stat on stat.playerID=DomNDuel.playerID) DomNDuel
    WHERE UTCSeconds=min_date) min_date

    ON max_date.username=min_date.username AND max_date.name=min_date.name
    ) wins_and_losses
    ...
    returnList = {}
    for user in ans:
        if user[1] not in usersList:
            usersList[user[1]] = [0,0]
        if user[0] == "Dominion":
            usersList[user[1]][0] = user[2]
        else:
            usersList[user[1]][1] = user[2]
    
    if DuelFlag:
        for user in usersList:
            if usersList[user][1] > usersList[user][0] * proportion and usersList[user][1] > 10 and usersList[user][0] > 10:
                returnList[user] = usersList[user]
    else:
        for user in usersList:
            if usersList[user][0] > usersList[user][1] * proportion and usersList[user][1] > 10 and usersList[user][0] > 10:
                returnList[user] = usersList[user]
...
usersToUse = getUsersByModeProp(DuelFlag=True,proportion=2)
...
for i in range(len(ans)):
    ...
    if user in activeUsers and user in usersToUse:
